...
public class Staging {
    ...
    public void execute() {
        ...
        try {
            ...
            // success
            // 8.2. Check if the StagingNews table exists
            // false
            if (!doesTableExist(connectionStaging, "StagingNews")) {
                // 8.2.1. Create table StagingNews: id, title, image, category, desciption, content, author, tags, create_at, update_at, create_by, update_by
                createStagingNewsTable(connectionStaging);
            }
            // true
            else {

                // 8.2.2. Truncate table StagingNews
                truncateTable();
            }

            // 9. Get the folder containing files from the location attribute of the data_file_configs table (select location form control.data_files_configs where create_at = CURRENT_DATE() order by create_at DESC limit 1) + VNEXPRESS_dd_MM__.csv
            ...
            // 12. Insert data from VNEXPRESS_dd_MM_yyyy.csv into the StagingNews table: title, image, category, desciption, content, author, tags, source, create_at, update_at, create_by, update_by
            insertDataIntoStagingNews(filePath);
    ...
    public void createStagingNewsTable(Connection connection) throws SQLException {
        ...
        statement.executeUpdate("CREATE TABLE staging.StagingNews (id int auto_increment PRIMARY KEY, title TEXT, image TEXT, category TEXT, " +
                "desciption TEXT, content TEXT, author TEXT, tags TEXT, create_at TEXT, update_at TEXT, create_by TEXT," +
                " update_by TEXT)");
        statement.close();
    }
